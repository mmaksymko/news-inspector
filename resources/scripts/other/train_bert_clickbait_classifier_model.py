...
import os
save_directory = 'bert_clickbait_model_v1'
...
from torch.optim.lr_scheduler import ReduceLROnPlateau, CosineAnnealingLR, StepLR, ExponentialLR, CyclicLR, OneCycleLR, CosineAnnealingWarmRestarts
...
types = {'title': str, 'label': int}
dataset_location = r"M:\Personal\SE\bachelors\python\scrape\training_data\processed_clickbait_titles.csv"

# Load dataset
df = pd.read_csv(dataset_location, dtype=types, header=None, names=['headline', 'label'])
...
def get_loader(df, content_column, label_column, tokenizer, max_len, batch_size):
    dataset = ClickbaitDataset(df[content_column].tolist(), df[label_column].tolist(), tokenizer, max_len)
    return DataLoader(dataset, batch_size=batch_size, shuffle=True)

# Create DataLoaders
train_loader = get_loader(train_df, 'headline', 'label', tokenizer, MAX_LEN, BATCH_SIZE)
val_loader = get_loader(val_df, 'headline', 'label', tokenizer, MAX_LEN, BATCH_SIZE)
test_loader = get_loader(test_df, 'headline', 'label', tokenizer, MAX_LEN, BATCH_SIZE)
