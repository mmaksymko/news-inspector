...
from transformers import ElectraTokenizer, ElectraForSequenceClassification, get_linear_schedule_with_warmup, ElectraConfig
...
def visualize(train_metrics, val_metrics):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_directory}/metrics_electra_multilabel.png')
    plt.show()

set_seed(42)

MODEL_NAME = 'google/electra-base-discriminator'  # Using ELECTRA instead of RoBERTa
DROPOUT = 0
BATCH_SIZE = 4
...
config = ElectraConfig.from_pretrained(MODEL_NAME, num_labels=len(types.keys())-1, attention_probs_dropout_prob=DROPOUT, hidden_dropout_prob=DROPOUT)
tokenizer = ElectraTokenizer.from_pretrained(MODEL_NAME)
model = ElectraForSequenceClassification.from_pretrained(MODEL_NAME, config=config).to(device)

save_directory = f'new_dataset_electra_propaganda_multilabel_model_v2_removed_columns_v4'
os.makedirs(save_directory, exist_ok=True)
df = read_csv(fr'M:\Personal\SE\bachelors\python\processed_propaganda\propaganda_with_examples_headlines_v11-170408_probably_final_removed_v2.csv', types, header=True)
